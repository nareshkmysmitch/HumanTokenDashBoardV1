...
class ProductDetailScreen(
    private val product: Product, private val viewModel: MarketPlaceViewModel
) : Screen {
        ...
        Scaffold(
            topBar = {
                TopAppBar(
                    ...
                    actions = {
                        IconButton(onClick = { navigator.push(CartScreen(viewModel)) }) {
                            Icon(
            ...
            when (productDetailsState) {
                ...
                is ProductDetailsState.Success -> {
                    val currentProduct =
                        (productDetailsState as ProductDetailsState.Success).product
                    Column(
                        ...
                        Button(
                            onClick = {
                                if (buttonText != "Successfully added to cart") {
                                    ...
                                    val variantId =
                                        currentProduct.variants?.firstOrNull()?.variant_id
                        ...
                        currentProduct.stock?.let {
                            if (it > 0) {
                                Row(
                                    ...
                                    Text(
                                        "Only ${currentProduct.stock} left", color = AppColors.White
                                    )
                                }
                            }
                        }
...
private fun InfoRow(icon: ImageVector, text: String) {
    Row(
        ...
        Spacer(modifier = Modifier.width(8.dp))
        Text(
            text = text, fontFamily = FontFamily.medium(), color = AppColors.textSecondary
        )
    }
}
